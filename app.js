...
function updateDateTime() {
  ...
  // Update Kotak Tanggal di bawahnya
  const elDate = document.getElementById('date-text');
  if (elDate) {
    elDate.textContent = `${dayName}, ${date} ${month} ${year}`;
  }

  // Fallback (jaga-jaga elemen lama masih ada)
  const oldEl = document.getElementById('datetime');
  if (oldEl && !document.querySelector('.custom-home')) {
    oldEl.textContent = `${dayName}, ${date} ${month} ${year} (${hh}:${mm}:${ss} WIB)`;
  } else if (oldEl) {
    oldEl.style.display = 'none'; // Sembunyikan yang lama di home baru
  }
}
...
// Update navigasi untuk menambahkan halaman About Me
